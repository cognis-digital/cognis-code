"""Native cognis-connect emit for cognis-code — forward findings to any platform.

Maps cognis-code's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    cognis-code ... --format json | cognis-code-emit --to stix
    cognis-code-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
