...
def _continue_cfg(endpoint, coder, reasoner):
    ...
    return {
        "models": [
            {"title": "Cognis Coder (local, uncensored)", "provider": "openai", "model": coder,
             "apiBase": base, "apiKey": "local"},
            {"title": "Cognis Reasoner (local)", "provider": "openai", "model": reasoner,
             "apiBase": base, "apiKey": "local"},
        ],
        "tabAutocompleteModel": {"title": "Coder", "provider": "openai", "model": coder,
                                 "apiBase": base, "apiKey": "local"},
    }


def write_ide_config(ide: str, endpoint: str = DEFAULT_ENDPOINT, dry_run: bool = False) -> dict:
    """Write (or preview) the config for one IDE/agent. Returns {path, content}."""
    coder = resolve("coder"); reasoner = resolve("reasoner")
    ide = ide.lower()
    if ide == "continue":  # VS Code + JetBrains (Continue.dev)
        ...
        content = json.dumps({"provider": {"local": {"npm": "@ai-sdk/openai-compatible",
            "options": {"baseURL": endpoint}, "models": {coder: {}, reasoner: {}}}},
            "model": f"local/{coder}"}, indent=2)
    elif ide == "aider":
        p = HOME / ".aider.conf.yml"
        content = (f"openai-api-base: {endpoint}\nopenai-api-key: local\n"
                   f"model: openai/{coder}\nweak-model: openai/{coder}\n")
    elif ide == "zed":
        p = HOME / ".config" / "zed" / "settings.json"
        content = json.dumps({"assistant": {"default_model": {"provider": "openai", "model": coder},
            "version": "2"}, "language_models": {"openai": {"api_url": endpoint, "available_models": [
            {"name": coder, "max_tokens": 32768}]}}}, indent=2)
    elif ide == "cursor":
        p = HOME / ".cognis-code" / "cursor-instructions.md"
        content = ("# Cursor → local uncensored model\n\n"
                   "Settings → Models → enable 'Override OpenAI Base URL':\n"
                   f"- Base URL: `{endpoint}`\n- API key: `local`\n- Model: add `{coder}`\n")
    elif ide == "neovim":
        p = HOME / ".cognis-code" / "neovim-avante.lua"
        content = ("-- Avante.nvim (or codecompanion) → local endpoint\n"
