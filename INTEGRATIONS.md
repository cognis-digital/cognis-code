...
**cognis-code** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `cognis-code` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
cognis-code ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
cognis-code ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
cognis-code ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `cognis-code` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="cognis-code") for rec in records]   # records = your JSON output
